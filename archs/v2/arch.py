...
class ResNetAttentionEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.layer1 = self._make_layer(
            channels[0], channels[0], stride=1, dropout=dropout
        )
        self.layer2 = self._make_layer(
            channels[0], channels[1], stride=2, dropout=dropout
        )
        self.layer3 = self._make_layer(
            channels[1], channels[2], stride=2, dropout=dropout
        )
        self.layer4 = self._make_layer(
            channels[2], channels[3], stride=2, dropout=dropout
        )
...
class ProtoNetV2(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(distance, str):
            ...
            self.distance_type = (
                distance.value if hasattr(distance, "value") else str(distance).lower()
            )
            self.distance_metric = None
