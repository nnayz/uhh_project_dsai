...
class ListData:
    ...
    def list_directories(self) -> None:
        for dir in self.data_dir.iterdir():
            if (
                ...
                and not self._is_virtual_env(dir)
            ):
                print(dir.name)
    ...
    def list_training_directories(self) -> None:
        for dir in self.data_dir.iterdir():
            if (
                dir.is_dir()
                and dir.name.startswith("Training")
                and not self._is_virtual_env(dir)
            ):
                for subdir in dir.iterdir():
                    if (
                        ...
                        and not self._is_virtual_env(subdir)
                    ):
                        print(
                            f"{self.directories_mapping.get(subdir.name, subdir.name)}: {subdir.name}"
                        )
    ...
    def list_validation_directories(self) -> None:
        for dir in self.data_dir.iterdir():
            if (
                dir.is_dir()
                and dir.name.startswith("Validation")
                and not self._is_virtual_env(dir)
            ):
                for subdir in dir.iterdir():
                    if (
    ...
    def list_evaluation_directories(self) -> None:
        for dir in self.data_dir.iterdir():
            if (
                dir.is_dir()
                and dir.name.startswith("Evaluation")
                and not self._is_virtual_env(dir)
            ):
                for subdir in dir.iterdir():
                    if (
